...
class MCPHTTPClient:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            raise MCPConnectionError("aiohttp is required for HTTP MCP clients. Install with: pip install aiohttp")
        except Exception as e:
                        ...
                        if response.status == 200:
                            ...
                            if not response_data:
                                ...
                                try:
                                    ...
                                    last_error = f"Could not parse response: {response_text}"
                                    continue
                            ...
                            last_error = f"HTTP error {response.status}: {await response.text()}"
                            continue
