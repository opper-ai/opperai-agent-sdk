...
class FunctionTool(Tool):
    ...
    def __init__(self, func: Callable, name: str = None, description: str = None, parameters: Dict[str, str] = None):
...
def tool(func: Callable = None, *, name: str = None, description: str = None, parameters: Dict[str, str] = None):
    """
...
class Agent:
    ...
    def __init__(
        ...
        if tools is not None and flow is not None:
            raise ValueError("Cannot specify both 'tools' and 'flow' parameters. Choose one mode of operation.")
        ...
        try:
            ...
            if hasattr(self.flow.input_model, 'model_validate'):
                ...
                try:
                    ...
                    if hasattr(self.flow.input_model, '__annotations__') and 'goal' in self.flow.input_model.__annotations__:
                        ...
                        conversion_result = self.call_llm(
                            ...
                            input_data={"goal": goal, "required_fields": list(self.flow.input_model.__annotations__.keys())},
                            output_schema=self.flow.input_model,
                            parent_span_id=trace.id
                        )
                        flow_input = self.flow.input_model.model_validate(conversion_result.json_payload)
                except Exception as e:
                    ...
                    try:
                        ...
                        if 'ingredients' in self.flow.input_model.__annotations__:
                            ...
                            ingredient_patterns = re.findall(r'(?:\d+\w*\s+)?[a-zA-Z][a-zA-Z\s]+(?=,|$|and)', goal)
                            if ingredient_patterns:
                                ingredients = [ing.strip() for ing in ingredient_patterns]
                                ...
                                ingredients = [part.strip() for part in parts if part.strip() and not part.startswith('create')]
                                if ingredients:
                                    flow_input = self.flow.input_model(ingredients=ingredients)
                                else:
                        ...
                        raise ValueError(f"Cannot convert goal '{goal}' to flow input model {self.flow.input_model.__name__}. "
