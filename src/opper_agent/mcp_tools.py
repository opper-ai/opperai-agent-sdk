...
class MCPToolAdapter:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.info(f"Connected to MCP server '{self.server_config.name}' with {len(self._tools_cache)} tools")
    ...
    def _convert_mcp_tool(self, mcp_tool: MCPToolSchema) -> FunctionTool:
        ...
        if hasattr(input_model, '__fields__'):
            for field_name, field_info in input_model.__fields__.items():
                parameters[field_name] = field_info.description or f"Parameter {field_name}"
...
def create_mcp_tools(server_configs: List[MCPServerConfig]) -> Callable[[], List[FunctionTool]]:
    """
    ...
    def get_tools() -> List[FunctionTool]:
        ...
        try:
            ...
            if loop.is_running():
                ...
                logger.warning("Cannot initialize MCP tools inside an event loop. Initialize MCP tools before creating the agent.")
                return []
            else:
