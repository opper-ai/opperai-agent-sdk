...
@tool
def filter_numbers(numbers: List[float], condition: str, threshold: float) -> List[float]:
    """
...
def create_math_agent(api_key=None):
    ...
    return Agent(
        ...
        tools=[calculate, square_root, factorial, sort_numbers, find_statistics, filter_numbers],
...
def create_general_assistant(api_key=None):
    ...
    all_tools = [
        # Math tools
        calculate, square_root, factorial,
        # Text tools
        count_words, count_characters, reverse_text, find_longest_word,
        # Data tools
        sort_numbers, find_statistics, filter_numbers,
        # Time tools
        get_current_time, days_between_dates,
        # File tools
        list_files_in_directory, read_text_file
    ]
